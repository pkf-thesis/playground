...
path = "/30T/Music/MTAT/audio"
feature_path = "/30T/Music/MSD/npzs/mtat/"
dataset = "mtat"
max_length = 640512
frequency = 22050
dir_name = ''
...
def convert_files():
    ...
    train_ids = [song.rstrip() for song in open("../data/%s/train.txt" % dataset)]
    # loaded_train_ids = [song.split('/')[-1].rstrip() for song in open("../data/msd/train_path.txt")]
    convert_list(train_ids, "../data/%s/train_path.txt" % dataset, count)

    print("Converting valid data")
    valid_ids = [song.rstrip() for song in open("../data/%s/valid.txt" % dataset)]
    convert_list(valid_ids, "../data/%s/valid_path.txt" % dataset, count)

    print("Converting test data")
    test_ids = [song.rstrip() for song in open("../data/%s/test" % dataset)]
    convert_list(test_ids, "../data/%s/test_path.txt" % dataset, count)
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            if splitted_file[0] in list_ids:
                ...
                if count % 1000 == 0: dir_name = "%s-%s" % (count, count + 1000)
